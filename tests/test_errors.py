...
describe "DelfickError":
    ...
    it "can format special values":
        ...
        assert error.as_dict() == {"blah": "formatted_blah_1", "meh": "formatted_meh_2", "things": 3}

        assert error.as_tuple()[2] == (("blah", wf1), ("meh", wf2), ("things", 3))
        assert error.as_tuple(formatted=True)[2] == (("blah", "formatted_blah_1"), ("meh", "formatted_meh_2"), ("things", 3))
    ...
    describe "formatted_val":
        ...
        it "catches any exception from delfick_error_format":
            ...
            assert DelfickError().formatted_val(key, thing) == "<|Failed to format val for exception: val={0}, error={1}|>".format(thing, error)
            ...
            it "complains if any given kwargs doesn't match":
                ...
                for iterator, (part, val) in self.expecting_raised_assertion(called, Expected, one=1, two=1, three=1):
                    if part is InsideManager:
                        ...
                        assert str(val) == "Mismatched: {'three': expected=1, got=3}, {'two': expected=1, got=2}"
            ...
            it "complains about message before kwargs":
                ...
                for iterator, (part, val) in self.expecting_raised_assertion(called, Raised, "testing, 1.. 2.. 3", one=1, three=2):
                    if part is InsideManager:
                        ...
                        assert str(val) == "Regex didn't match: 'testing, 1.. 2.. 3' not found in 'testing for great good'"
