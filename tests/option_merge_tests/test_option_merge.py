...
describe TestCase, "MergedOptions":
    ...
    it "has classmethod for adding options":
        ...
        self.assertEqual(
            merged.storage.data, [([], options2, "somewhere"), ([], options1, "somewhere")]
        )
    ...
    describe "Getting all values for a key":
        it "finds all the values":
            ...
            values = list(self.merged.values_for("a"))
            self.assertEqual(
                values, [(400, False), ({"c": 5}, False), ({"c": 4}, False), (1, False)]
            )
    ...
    describe "Getting items":
        it "combines everything into one key,value list":
            ...
            self.merged["a"] = {"c": 5, "d": 8}
            self.assertEqual(
                sorted(self.merged.items()),
                sorted({"b": 4, "a": self.merged.prefixed("a")}.items()),
            )
            ...
            self.assertEqual(
                sorted((k, dict(v.items())) for k, v in self.merged.items()),
                sorted({"b": {"c": 9}, "a": {"c": 5, "d": 8}}.items()),
            )
...
describe TestCase, "Converters":
    ...
    describe "KeyValuePairsConverter":
        ...
        it "converts complex key value pairs into a dictionary":
            ...
            self.assertEqual(
                dict(converted["five"].items()), {"six": converted["five"].prefixed("six")}
            )
            six = converted["five"]["six"]
            self.assertEqual(
                dict(converted["five"]["six"].items()), {"seven": "eight", "nine": "ten"}
            )
        ...
        it "Overrides previous keys":
            ...
            self.assertEqual(
                dict(converted["one"].items()), {"two": converted["one"].prefixed("two")}
            )
    ...
    describe "AttributesConverter":
        it "converts all attributes to a dictionary":
            ...
            self.assertEqual(
                dict(converted.items()), {"one": "two", "three": "four", "b": obj.b, "a": obj.a}
            )
        ...
        it "only includes specified attributes if specified":
            ...
            self.assertEqual(
                dict(converter.convert().items()), {"one": None, "hi": "hello", "__class__": Obj}
            )
        ...
        it "can lift the result if provided with a prefix to lift against":
            ...
            self.assertEqual(
                dict(converted["cats"]["pandas"].items()),
                {"one": "two", "hi": "hello", "__class__": Obj},
            )
